...
class reg_slave_ID extends uvm_reg;
   ...
   // name, n_bits, has_coverage
   function new(string name = "slave_ID");
      super.new(name,32,UVM_NO_COVERAGE);
   endfunction

   // parent, size, lsb_pos, access, volatile, reset, has_reset, is_rand, individually accessible
...
`endif
      ...
      byte_addressing specifies whether consecutive addresses refer are 1 byte apart (TRUE) or n_bytes apart (FALSE).  Default is TRUE.Create an address map with the specified name, then configures it with the following properties.
